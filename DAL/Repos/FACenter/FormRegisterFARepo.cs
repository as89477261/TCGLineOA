...
{
    public class FormRegisterFARepo
    {
        ...
        public string InsertFATransections(FormRegisterFAInfo obj, int ConsultantId)
        {
            ...
            try
            {
                ...
                list.Add(new SqlParameter("InputChannel",
                    5)); //FA ต้องการแยก Channal แหล่งที่มาให้ชัดเจน พี่รันแจ้งปรับเป็น InputChannel = 5 
        ...
        public string InsertFormRegister(FormRegisterFAInfo reg)
        {
            ...
            try
            {
                //if (reg.Id <= 0)
                //{
                //    string fa_id = InsertFATransections(reg.ApplicationId, reg.CustomerProfileId, ConsultantId, reg.FA_Ref, reg.CurrentStatusId);
                //    if (string.IsNullOrEmpty(fa_id))
                //    {
                //        return string.Empty;
                //    }
                //    reg.FA_TransectionsId = int.Parse(fa_id);
                //}
                //if (reg.FA_TransectionsId <= 0) return string.Empty;
