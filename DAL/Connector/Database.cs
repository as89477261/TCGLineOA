...
public class Database
{
    ...
    public Database(DBName DbName)
    {
        ...
        switch (DbName)
        {
            ...
            case DBName.DB_SMEClinic:
                ...
                break;
            case DBName.DB_FACenter:
                ConnectionString = ConfigurationManager.AppSettings["DB_FACenter"];
                break;
            case DBName.DB_TCGID:
                ConnectionString = ConfigurationManager.AppSettings["DB_TCGID"];
                break;
            case DBName.DB_CYBER:
                ConnectionString = ConfigurationManager.AppSettings["DB_CYBER"];
                break;
            case DBName.DB_NCB:
                ConnectionString = ConfigurationManager.AppSettings["DB_NCB"];
                break;
            default:
                break;
        }
    ...
    public string ExecuteStoredProcedureReturnValue(string StoredProcedureName)
    {
        var connectToDB = new SqlConnection(ConnectionString);
        var cmdExecution = new SqlCommand(StoredProcedureName, connectToDB);
        cmdExecution.CommandType = CommandType.StoredProcedure;
        ...
        {
            sqlAdapter.Fill(datasetResult);
            if (datasetResult.Tables[0] != null && datasetResult.Tables[0].Rows.Count > 0)
            {
                var bufferResult = datasetResult.Tables[0].Rows[0][0].ToString();
    ...
    public string ExecuteStoredProcedureReturnStatus(string StoredProcedureName, SqlParameter[] parameters)
    {
        var connectToDB = new SqlConnection(ConnectionString);
        var cmdExecution = new SqlCommand(StoredProcedureName, connectToDB);
        cmdExecution.CommandType = CommandType.StoredProcedure;
