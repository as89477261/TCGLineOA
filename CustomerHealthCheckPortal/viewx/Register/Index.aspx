...
CodeBehind="Index.aspx.cs" Inherits="CustomerHealthCheck.viewx.Register.Index" %>
            ...
            <div class="col-12">
                <div class="content mt-0">
                    ...
                    <div class="form-custom form-label form-border mb-3 bg-transparent">
                        <input type="text" class="form-control rounded-xs" id="txtBusinessName" placeholder="โปรดระบุชื่อกิจการ">
                        <label for="txtBusinessName" class="color-highlight form-label-always-active">ชื่อกิจการ</label>
                        <span>(required)</span>
                    </div>
                    <div class="form-custom form-label form-border mb-3 bg-transparent">
                        <input type="text" class="form-control rounded-xs" id="txtFirstName" placeholder="โปรดระบุชื่อ">
                        <label for="txtFirstName" class="form-label-always-active color-highlight">ชื่อ</label>
                        <span>(required)</span>
                    </div>
                    <div class="form-custom form-label form-border mb-3 bg-transparent">
                        <input type="text" class="form-control rounded-xs" id="txtLastName" placeholder="โปรดระบุนามสกุล">
                        <label for="txtLastName" class="form-label-always-active color-highlight">นามสกุล</label>
                        <span>(required)</span>
                    </div>
                    <div class="form-custom form-label form-border mb-3 bg-transparent">
                        <input type="text" class="form-control rounded-xs" id="txtPosition" placeholder="โปรดระบุตำแหน่ง">
                        <label for="txtPosition" class="form-label-always-active color-highlight">ตำแหน่ง</label>
                        <span>(required)</span>
                    </div>
                    <div class="form-custom form-label form-border mb-3 bg-transparent">
                        <input type="number" class="form-control rounded-xs" id="txtPhone" placeholder="โปรดระบุเบอร์มือถือ" maxlength="10">
                        <label for="txtPhone" class="color-highlight form-label-always-active">เบอร์มือถือ</label>
                        <span>(required)</span>
                    </div>
                    <div class="form-custom form-label form-border mb-3 bg-transparent">
                        <input type="email" class="form-control rounded-xs" id="txtEmail" placeholder="โปรดระบุอีเมล์">
        ...
        <div class="row" id="step2" style="display: none;">
            <div class="col-12">
                <div class="content mt-0">
                    ...
                    <div class="form-custom form-label form-border form-icon">
                        <i class="bi bi-calendar font-13"></i>
                        <label for="c6a" class="color-highlight form-label-always-active">ลักษณะกิจการ</label>
                        <asp:DropDownList CssClass="form-select rounded-xs" runat="server"
                                          ID="ddlBusinessNature" ClientIDMode="Static" AppendDataBoundItems="true">
                    ...
                    <div class="form-custom form-label form-border mb-3 bg-transparent">
                        <input type="number" class="form-control rounded-xs" id="txtCompanyAge" placeholder="โปรดระบุอายุกิจการ" maxlength="5" onkeyup="return numberMobile(event)">
                        <label for="txtCompanyAge" class="form-label-always-active color-highlight">อายุกิจการ</label>
                        <span>(required)</span>
                    </div>
                    <div class="form-custom form-label form-border mb-3 bg-transparent">
                        <input type="text" class="form-control rounded-xs" id="txtIncomePerYear" placeholder="โปรดระบุยอดขายต่อปี" onkeyup="return numberMobile(event)">
                        <label for="txtIncomePerYear" class="form-label-always-active color-highlight">ยอดขายต่อปี</label>
                        <span>(required)</span>
                    </div>
                    <div class="form-custom form-label form-border form-icon">
                        <i class="bi bi-calendar font-13"></i>
                        <label for="c6a" class="color-highlight form-label-always-active">Statement ที่ใช้บัญชีหลักกับธนาคาร </label>
                        <asp:DropDownList CssClass="form-select rounded-xs" runat="server"
                                          ID="ddlStatementBank" ClientIDMode="Static" AppendDataBoundItems="true">
                            <asp:ListItem Value="" Text="- กรุณาเลือก -"/>
                        </asp:DropDownList>

                    </div>
                    <%-- <div class="form-custom form-label form-border mb-3 bg-transparent">
                                <input type="text" class="form-control rounded-xs" id="txtBranch" placeholder="โปรดระบุสาขา">
                                <label for="txtBranch" class="form-label-always-active color-highlight">สาขา</label>
                                <span>(required)</span>
                            </div>--%>
                    <div class="form-custom form-label form-border form-icon">
                        <i class="bi bi-calendar font-13"></i>
                        <label for="c6a" class="color-highlight form-label-always-active">วัตถุประสงค์ในการขอสินเชื่อครั้งนี้</label>
                        <select class="form-select rounded-xs" id="ddlPurpose">
                            <option value="" selected="">- กรุณาเลือก -</option>
                            <option value="001">หมุนเวียนในกิจการ</option>
                            <option value="002">ลงทุนในกิจการ</option>
                            <option value="003">Refinance</option>
                        </select>
                    </div>
                    <div class="form-custom form-label form-border mb-3 bg-transparent">
                        <input type="text" class="form-control rounded-xs" id="txtLoanMoney" placeholder="โปรดระบุวงเงินสินเชื่อที่ต้องการ (ล้านบาท)" onkeyup="return numberMobile(event)">
                        <label for="txtLoanMoney" class="form-label-always-active color-highlight">วงเงินสินเชื่อที่ต้องการ (ล้านบาท)</label>
                        <span>(required)</span>
                    </div>
                    <div class="form-custom form-label form-border form-icon">
                        <i class="bi bi-calendar font-13"></i>
                    ...
                    <div class="form-custom form-label form-border form-icon">
                        <i class="bi bi-calendar font-13"></i>
                        <label for="c6a" class="color-highlight form-label-always-active">มีหลักทรัพย์ค้ำประกันหรือไม่</label>
                        <select class="form-select rounded-xs" id="ddlHaveAssetGarantee" onchange="ToggleMainAssetValue();">
                            <option value="" selected="">- กรุณาเลือก -</option>
                            <option value="001">ไม่มี</option>
                            <option value="002">มี (โปรดระบุ) </option>
                        </select>
                    </div>
                    <div id="divHaveAssetGaranteeValue" class="form-custom form-label form-border mb-3 bg-transparent" style="display: none;">
                        <input type="text" class="form-control rounded-xs" id="txtHaveAssetGaranteeValue" placeholder="โปรดระบุหลักทรัพย์ค้ำประกัน">
                        <label for="txtHaveAssetGaranteeValue" class="form-label-always-active color-highlight">โปรดระบุหลักทรัพย์ค้ำประกัน</label>
                        <span>(required)</span>
                    </div>
                    ...
                    <div class="form-custom form-label form-border form-icon">
                        <i class="bi bi-calendar font-13"></i>
                        <label for="c6a" class="color-highlight form-label-always-active">ครั้งนี้สนใจขอสินเชื่อผ่านธนาคารใด</label>
                        <asp:DropDownList CssClass="form-select rounded-xs" runat="server"
                                          ID="ddlPurpostFromBank" ClientIDMode="Static" AppendDataBoundItems="true">
                            <asp:ListItem Value="" Text="- กรุณาเลือก -"/>
                        </asp:DropDownList>
                    </div>
                </div>
