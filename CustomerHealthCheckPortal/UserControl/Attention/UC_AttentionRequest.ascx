<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UC_AttentionRequest.ascx.cs" Inherits="CustomerHealthCheck.UserControl.Attention.UC_AttentionRequest" %>

<link rel="stylesheet" type="text/css" href="<%= ResolveClientUrl("~/styles/AttentionWizard.css") + "?v=" + DateTime.Now.ToString("ddMMyyyyHHmmss") %>">


<asp:Literal Text="" ID="Header" runat="server"/>

<asp:Literal Text="" ID="SubHeader" runat="server"/>

<div class="row">
    <h3>สถานะลงทะเบียน</h3>
    <div class="column-md-1 verticalWizard" style="z-index:0;">
        <div class="verticalWizard">
            <ul style="margin:0px;">
                <asp:Literal Text="" ID="ltlStep" runat="server"/>

                <%-- <li class="complete" data-target="#step5">
                                        <a href="#tab5" data-toggle="tab" class="active" aria-expanded="true">
                                            <span class="round-tab"></span>
                                            <span class="title">Step 1</span>
                                        </a>
                                    </li>
                                    <li class="active" data-target="#step6">
                                        <a href="#tab6" data-toggle="tab" aria-expanded="false">
                                            <span class="round-tab"></span>
                                            <span class="title">Step 2</span>
                                        </a>
                                    </li>
                                    <li class="reject" data-target="#step7">
                                        <a href="#tab7" data-toggle="tab" aria-expanded="false">
                                            <span class="round-tab"></span>
                                            <span class="title">Step 3</span>
                                        </a>
                                    </li>
                                    <li data-target="#step8">
                                        <a href="#tab8" data-toggle="tab" aria-expanded="false">
                                            <span class="round-tab"></span>
                                            <span class="title">Step 4</span>
                                        </a>
                                    </li>--%>
            </ul>
            <div class="clearfix"></div>
        </div>
    </div>
</div>
<asp:Literal Text="" ID="Footer" runat="server"/>




























<%--


       <div class="card card-style">
                <div class="content">

                    <div class="d-flex" style="margin-bottom: 10px;">
                        <div class="mt-1">

                            <h1 class="mb-0">ข้อมูลลงทะเบียนโครงการ PGS 10</h1>
                        </div>
                        <div class="ms-auto">
                            <i class="bi bi-clipboard-check-fill color-green-dark" style="font-size: 50px;"></i>
                        </div>
                    </div>
                    <div class="divider"></div>
                    <div class="row mb-3 mt-4">
                        <h5 class="col-4 text-start font-15">ชื่อผู้ติดต่อ</h5>
                        <h5 class="col-8 text-end font-14 opacity-60 font-400">
                            <asp:Label Text="" ID="ltlFullName" runat="server" />
                          
                        <h5 class="col-4 text-start font-15">ประเภทธุรกิจ</h5>
                        <h5 class="col-8 text-end font-14 font-400">
                            <asp:Literal Text="" ID="ltlBusinessType" runat="server" /></h5>
                        <h5 class="col-4 text-start font-15">วันที่ลงทะเบียน</h5>
                        <h5 class="col-8 text-end font-14 opacity-60 font-400">
                            <asp:Literal Text="" ID="ltlRegisterDate" runat="server" /></h5>

                    </div>


                    <style>
                        .wizard {
                            margin: 0 auto 15px;
                            width: 600px;
                        }

                            .wizard ul {
                                display: block;
                                list-style: none;
                                padding: 0;
                                position: relative;
                                width: 600px;
                            }

                                .wizard ul li {
                                    display: block;
                                    float: left;
                                    width: 25%;
                                    text-align: center;
                                    padding-left: 0;
                                }

                                    .wizard ul li:before {
                                        border-top: 2px solid #4990e2;
                                        content: "";
                                        display: block;
                                        font-size: 0;
                                        overflow: hidden;
                                        position: relative;
                                        top: 12px;
                                        right: 1px;
                                        width: 100%;
                                        z-index: 1;
                                        /*height: calc(100% - 25px);*/
                                    }

                                    .wizard ul li:first-child:before {
                                        left: 50%;
                                        max-width: 50%;
                                    }

                                    .wizard ul li:last-child:before {
                                        max-width: 50%;
                                        width: 50%;
                                    }

                                    .wizard ul li.active .round-tab {
                                        border-color: #4990e2;
                                    }

                                        .wizard ul li.active .round-tab:after {
                                            content: "";
                                            display: block;
                                            height: 12px;
                                            width: 12px;
                                            background-color: #4990e2;
                                            border-radius: 50%;
                                            position: relative;
                                            top: 4px;
                                            left: 4px;
                                        }

                                    .wizard ul li.active .title {
                                        color: #4990e2;
                                    }

                                    .wizard ul li.complete .round-tab {
                                        background: #fff url(data:image/svg+xml;base64,) no-repeat 50%;
                                        border-color: #4990e2;
                                    }

                                    .wizard ul li.complete .title {
                                        color: #4990e2;
                                    }

                                    .wizard ul li.reject .round-tab {
                                        background: #fff url(data:image/svg+xml;base64,) no-repeat 50%;
                                        border-color: #F65F6E;
                                    }

                                    .wizard ul li.reject .title {
                                        color: #4990e2;
                                    }

                                    .wizard ul li .round-tab {
                                        width: 24px;
                                        height: 24px;
                                        line-height: 24px;
                                        display: inline-block;
                                        border-radius: 50%;
                                        background-color: #fff;
                                        border: 2px solid #adbec5;
                                        z-index: 2;
                                        position: relative;
                                        text-align: center;
                                    }

                                    .wizard ul li .title {
                                        color: #8ca4af;
                                        display: block;
                                        font-size: 12px;
                                        line-height: 12px;
                                        max-width: 100%;
                                        position: relative;
                                        table-layout: fixed;
                                        text-align: center;
                                        top: 7px;
                                        word-wrap: break-word;
                                        z-index: 104;
                                    }

                        .verticalWizard {
                            margin: 0 auto 10px;
                            margin-top:5px;
                        }

                            .verticalWizard ul {
                                display: block;
                                list-style: none;
                                padding: 0;
                                position: relative;
                               /* width: 600px;*/
                            }

                                .verticalWizard ul li {
                                    display: block;
                                    position: relative;
                                }

                                    .verticalWizard ul li:before {
                                        content: '';
                                        position: absolute;
                                        width: 2px;
                                        height: calc(100%);
                                        background-color: #298FC2;
                                        left: 12px;
                                        bottom: -9px;
                                    }

                                    .verticalWizard ul li:last-child:before {
                                        display: none;
                                    }

                                    .verticalWizard ul li.active .round-tab {
                                        border-color: #4990e2;
                                    }

                                        .verticalWizard ul li.active .round-tab:after {
                                            content: "";
                                            display: block;
                                            height: 12px;
                                            width: 12px;
                                            background-color: #4990e2;
                                            border-radius: 50%;
                                            position: relative;
                                            top: 4px;
                                            left: 4px;
                                        }

                                    .verticalWizard ul li.active .title {
                                        color: #4990e2;
                                    }

                                    .verticalWizard ul li.complete .round-tab {
                                        background: #fff url(data:image/svg+xml;base64,) no-repeat 50%;
                                        border-color: #4990e2;
                                    }

                                    .verticalWizard ul li.complete .title {
                                        color: #4990e2;
                                    }

                                    .verticalWizard ul li.reject .round-tab {
                                        background: #fff url(data:image/svg+xml;base64,) no-repeat 50%;
                                        border-color: #F65F6E;
                                    }

                                    .verticalWizard ul li.reject .title {
                                        color: #4990e2;
                                    }

                                    .verticalWizard ul li .round-tab {
                                        width: 24px;
                                        height: 24px;
                                        line-height: 24px;
                                        display: inline-block;
                                        border-radius: 50%;
                                        background-color: #fff;
                                        border: 2px solid #adbec5;
                                        z-index: 2;
                                        position: relative;
                                        text-align: center;
                                    }

                                    .verticalWizard ul li .title {
                                        color: #8ca4af;
                                        display: block;
                                        font-size: 16px;
                                        line-height: 12px;
                                        max-width: 90%;
                                        position: relative;
                                        table-layout: fixed;
                                        text-align: left;
                                        top: -22px;
                                        word-wrap: break-word;
                                        z-index: 104;
                                        left: 35px;
                                    }
                    </style>

                    <div class="row">
                        <h3>สถานะคำขอ</h3>
                        <div class="column-md-1 verticalWizard">
                            <div class="verticalWizard">
                                <ul>
                                    <asp:Literal Text="" ID="ltlStep" runat="server" />

                                   <%-- <li class="complete" data-target="#step5">
                                        <a href="#tab5" data-toggle="tab" class="active" aria-expanded="true">
                                            <span class="round-tab"></span>
                                            <span class="title">Step 1</span>
                                        </a>
                                    </li>
                                    <li class="active" data-target="#step6">
                                        <a href="#tab6" data-toggle="tab" aria-expanded="false">
                                            <span class="round-tab"></span>
                                            <span class="title">Step 2</span>
                                        </a>
                                    </li>
                                    <li class="reject" data-target="#step7">
                                        <a href="#tab7" data-toggle="tab" aria-expanded="false">
                                            <span class="round-tab"></span>
                                            <span class="title">Step 3</span>
                                        </a>
                                    </li>
                                    <li data-target="#step8">
                                        <a href="#tab8" data-toggle="tab" aria-expanded="false">
                                            <span class="round-tab"></span>
                                            <span class="title">Step 4</span>
                                        </a>
                                    </li>
                                </ul>
                                <div class="clearfix"></div>
                            </div>
                        </div>
                    </div>



                </div>
            </div>--%>