...
function redirectToContactUs() {
    url = "https://www.tcg.or.th/contactus_form.php?Channel=RegisterDC";
...
document.addEventListener("DOMContentLoaded", () => {
    const container1 = document.getElementById("ConfirmRegisterModalDebt");
    ...
    if (container1) {
        modalConfirmTCC = new bootstrap.Modal(container1);
    }
    if (container2) {
        modalConfirmTCC = new bootstrap.Modal(container2); 
    }
    if (container3) {
        modalConfirmTCC = new bootstrap.Modal(container3);
    }
});

function OpenConfirmRegisterDebt() {

    if (ValidateForm() === true) {
        if (modalConfirmTCC) {  // ตรวจสอบว่ามี Instance ของ Modal หรือไม่
            modalConfirmTCC.show();
        } else {
            console.error("ไม่พบ Modal ที่มี ID: checkRegisterModalDebt");
        }
    } else {
